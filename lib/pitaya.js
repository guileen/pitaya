...
var TCPRequest = require('./tcp_req');
var TCPResponse = require('./tcp_res');
require('./http_req');
require('./http_res');
...
var exports = module.exports = function(options) {
  ...
  function httpHandle(req, res) {
    ...
    var url = req.originalUrl = req.url; // fix for connect middleware
    // path, querystring
  ...
  function packetHandle(msg, socket, session) {
    var req = new TCPRequest(msg, socket, session);
    var res = new TCPResponse(socket, format);
    httpHandle(req, res);
  }

  function tcpHandle(socket) {
    util.parseLenBuff(socket);

    var session = {};

    socket.on('message', function(msgbuff) {
        var msg;
        if(format == 'msgpack') {
          msg = msgpack.unpack(msgbuff);
        } else {
          msg = JSON.parse(msgbuff.toString());
        }
        packetHandle(msg, socket, session);
    });
  };

  var app = {};

  app.__defineGetter__('httpServer', function() {
      return require('http').createServer(httpHandle);
  });

  app.__defineGetter__('httpsServer', function() {
      return require('https').createServer(httpHandle);
  });

  app.__defineGetter__('tcpServer', function() {
      return require('net').createServer(tcpHandle);
  });
  ...
  app.get = function() {
    if(arguments.length == 1) return app.settings[arguments[0]];
  ...
  function httpUse(middleware) {
    baseMiddlewares.push(function(req, res, next) {
  ...
  function makeHandler(options) {
    ...
    if(middlewares.length == 1) {
      return function(req, res) {
        req.params = params;
        req.next = errorHandler;
        res.next = errorHandler;
        // req, res, next
