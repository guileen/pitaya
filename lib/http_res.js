...
var res = module.exports = http.ServerResponse.prototype;
...
res.render = function(name, options, fn){
  ...
  // merge app.locals
  if(app.locals) utils.merge(opts, app.locals);
  // merge dynamic locals
  utils.merge(opts, this.locals);
  // merge render options
  utils.merge(opts, options);

  // render it
  app.render(name, opts, fn || send);

  function send(err, text){
    if(err) return self.next(err, self.req, self);
    self.end(text);
  }
